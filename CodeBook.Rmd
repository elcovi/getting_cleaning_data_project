...
* Used the following packages in my scripts:
...
####Clean up the variable names ("features"); i.e., no parenthesis, etc. (These names are very informative, no further cleaning necessary)
```{r}
varnames_clean <- gsub("[()]", "", ssvar_names$V2)
```


####Merge the two sets of data & factors(for test & training) into single data frames that will be combined later to form the main data frame, and add column (variable) names to them:
```{r}
...
####Substitute "allthe_activities" data from numbers to actual names & make it into a single-column data frame:
```{r}
activities <- join(allthe_activities, 
                   nameof_activity, 
                   by = "V1", 
                   type = "left")
...
* This is done by filtering the variable names in the "features.txt" data, now "ssvar_names"
...
####Combine the selected data with its factors (Subjects & Activity) into a single data frame:

```{r}
main_data <- cbind(allthe_subjects, 
                   activities, 
                   selected_data)
#str(main_data)
```
...
####Finally, reshape the data into a "tidy" dataset from which to obtain the average of each variable for each activity and each subject:
